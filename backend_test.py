...
class AssetInventoryAPITester:
    ...
    def test_email_configuration_system(self):
        ...
        # Test 1: Check if email configuration exists
        success, response = self.run_test(
        ...
        if success:
            if response and len(response) > 0:
                ...
                print(f"     Use SSL: {config.get('use_ssl', 'Not set')}")
                print(f"     From Email: {config.get('from_email', 'Not set')}")
                print(f"     From Name: {config.get('from_name', 'Not set')}")
                print(f"     Is Active: {config.get('is_active', 'Not set')}")
                
                # Store config for further testing
                self.test_data['email_config'] = config
                
                # Check for TLS/SSL conflict
                if config.get('use_tls') and config.get('use_ssl'):
                    print(f"   ⚠️ TLS/SSL CONFLICT DETECTED: Both use_tls and use_ssl are enabled!")
                elif not config.get('use_tls') and not config.get('use_ssl'):
                    print(f"   ⚠️ NO ENCRYPTION: Neither TLS nor SSL is enabled!")
                else:
                    print(f"   ✅ TLS/SSL configuration looks correct")
                
                # Check if configuration is active
                if not config.get('is_active', False):
                    print(f"   ❌ EMAIL CONFIGURATION IS INACTIVE!")
                else:
                    print(f"   ✅ Email configuration is active")
            else:
                print(f"   ❌ NO EMAIL CONFIGURATION FOUND IN DATABASE!")
                return False
        else:
            print(f"   ❌ Failed to retrieve email configuration")
            return False
        
        # Test 2: Test email configuration endpoint with test email
        test_email_data = {
        ...
        if success:
            ...
            print(f"   ❌ Email test endpoint failed or not available")
        
        # Test 3: Create a test asset requisition to trigger email notifications
        if 'asset_type_id' in self.test_data:
            from datetime import datetime, timedelta
            required_by_date = (datetime.now() + timedelta(days=7)).isoformat()
            ...
            email_test_requisition = {
                ...
                "request_type": "New Allocation",
                "request_for": "Self",
                "justification": "Testing email notification system - should trigger manager notification",
                "required_by_date": required_by_date
            }
            
            print(f"\n   📝 Creating test asset requisition to trigger email notifications...")
            success, response = self.run_test(
                "Create Asset Requisition (Email Trigger Test)",
                "POST",
                "asset-requisitions",
                200,
                data=email_test_requisition,
            ...
            if success:
                email_test_req_id = response['id']
                print(f"   ✅ Test requisition created: {email_test_req_id[:8]}...")
                print(f"   📧 This should have triggered an email notification to manager")
                
                # Check if requisition has manager details for email routing
                manager_id = response.get('manager_id')
                manager_name = response.get('manager_name')
                if manager_id and manager_name:
                    print(f"   ✅ Manager details found for email routing:")
                    print(f"     Manager ID: {manager_id}")
                    print(f"     Manager Name: {manager_name}")
                else:
                    print(f"   ⚠️ No manager details found - email routing may fail")
                
                # Store for approval test
                self.test_data['email_test_req_id'] = email_test_req_id
            else:
                print(f"   ❌ Failed to create test requisition for email testing")
        
        return True

    def test_email_notification_triggers(self):
        """Test all email notification trigger points"""
        print(f"\n📬 TESTING EMAIL NOTIFICATION TRIGGERS")
        print("=" * 50)
        
        if 'asset_type_id' not in self.test_data:
            print("❌ Skipping email trigger tests - no asset type available")
            return False
        
        # Trigger 1: Asset Request Creation (should email manager)
        from datetime import datetime, timedelta
        ...
        trigger_test_req = {
            ...
            "request_type": "New Allocation",
            "request_for": "Self",
            "justification": "Email trigger test - new asset request",
            "required_by_date": required_by_date
        }
        
        print(f"\n   🔔 Trigger 1: Asset Request Creation")
        success, response = self.run_test(
            "Create Asset Request (Email Trigger)",
            "POST",
            "asset-requisitions",
            200,
            data=trigger_test_req,
            user_role="Employee"
        )
        
        if success:
            trigger_req_id = response['id']
            print(f"   ✅ Asset request created: {trigger_req_id[:8]}...")
            print(f"   📧 Should trigger: Manager notification email")
            
            # Check if manager details are present for email
            if response.get('manager_id') and response.get('manager_name'):
                print(f"   ✅ Manager email target identified: {response.get('manager_name')}")
            else:
                print(f"   ⚠️ No manager found - email notification may not be sent")
        
        # Trigger 2: Manager Approval (should email employee and asset manager)
        if success and "Manager" in self.tokens:
            approval_data = {
                "action": "approve",
                "reason": "Approved for email notification trigger testing"
            }
            
            print(f"\n   🔔 Trigger 2: Manager Approval")
            success, response = self.run_test(
                "Manager Approval (Email Trigger)",
                "POST",
                f"asset-requisitions/{trigger_req_id}/manager-action",
                200,
                data=approval_data,
                user_role="Manager"
            )
            ...
            if success:
                print(f"   ✅ Manager approval completed")
                print(f"   📧 Should trigger:")
                print(f"     - Employee: Approval notification")
                print(f"     - Asset Manager: Routing notification")
                
                # Check routing details
                if response.get('assigned_to_name'):
                    print(f"   ✅ Asset Manager routing target: {response.get('assigned_to_name')}")
                else:
                    print(f"   ⚠️ No Asset Manager routing - notification may not be sent")
        
        return True

    def test_email_system_diagnostics(self):
        """Comprehensive email system diagnostics"""
        print(f"\n🔧 EMAIL SYSTEM DIAGNOSTICS")
        print("=" * 40)
        
        # Diagnostic 1: Check email configuration completeness
        if 'email_config' in self.test_data:
            config = self.test_data['email_config']
            ...
            if missing_fields:
                ...
                print(f"   ✅ All required email configuration fields present")
            ...
            if 'gmail' in smtp_server:
                expected_port = 587 if config.get('use_tls') else 465
                if smtp_port != expected_port:
                    print(f"   ⚠️ Gmail SMTP port mismatch: using {smtp_port}, expected {expected_port}")
                else:
                    print(f"   ✅ Gmail SMTP configuration looks correct")
            
            elif 'outlook' in smtp_server or 'office365' in smtp_server:
                if smtp_port != 587:
                    print(f"   ⚠️ Outlook/Office365 SMTP port should be 587, using {smtp_port}")
                else:
                    print(f"   ✅ Outlook/Office365 SMTP configuration looks correct")
        
        return True

    def run_email_investigation_tests(self):
        """Run comprehensive email notification system investigation"""
        print("🔥" * 80)
        print("📧 EMAIL NOTIFICATION SYSTEM INVESTIGATION")
        print("🔥" * 80)
        
        # Login as Administrator first
        if not self.test_login("admin@company.com", "password123", "Administrator"):
            print("❌ Failed to login as Administrator - cannot proceed with email investigation")
            return False
        
        # Also login other roles for testing
        self.test_login("manager@company.com", "password123", "Manager")
        self.test_login("employee@company.com", "password123", "Employee")
        self.test_login("assetmanager@company.com", "password123", "Asset Manager")
        ...
        # Run email investigation tests
        self.test_email_configuration_system()
        self.test_email_notification_triggers()
        self.test_email_system_diagnostics()
        
        # Print investigation summary
        print("\n" + "=" * 80)
        print("📧 EMAIL INVESTIGATION SUMMARY")
        print("=" * 80)
...
def run_asset_requisitions_requested_for_name_test():
    ...
    manager_login_success = tester.test_login("manager@company.com", "password123", "Manager")
